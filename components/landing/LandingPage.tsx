  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (res.success && res.session) {
        if (res.autoRoleSwitched && res.actualRole) {
          setActiveRole(res.actualRole);
        }
        setSession(res.session);
        setViewMode('dashboard');
        setSuccessMsg(res.message || `Welcome, ${res.session.user.name}!`);
      } else {
            ...
            <div className="mb-3">
              ...
              <div className="grid grid-cols-2 gap-2 p-1 bg-slate-200/70 rounded-2xl border border-slate-300">
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl font-bold text-xs sm:text-sm transition-all duration-150 select-none cursor-pointer ${activeRole === 'student'
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl font-bold text-xs sm:text-sm transition-all duration-150 select-none cursor-pointer ${activeRole === 'admin'
                      ? 'bg-black text-white shadow-sm scale-[1.01]'
                      : 'bg-transparent text-slate-600 hover:text-slate-900 hover:bg-white/60'
                    }`}
                >
